 ...
 *   · localStorage 'vitality:<userId>:profile' — the connector or a UI can
 *     retune without a code change; it wins over the defaults
 *
...
/** The profile: this user's localStorage override if valid, else defaults. */
export function profile(userId: string): Profile {
  const raw = readScoped(userId, 'profile')
  if (raw) {
    try {
...
export function saveProfile(userId: string, p: Profile): void {
  writeScoped(userId, 'profile', JSON.stringify(p))
}

/** Bodyweight for tile math (peak PK etc.) — 75 kg until they've been asked. */
export function bodyWeightKg(userId: string): number {
  return profile(userId).weightKg ?? 75
}
